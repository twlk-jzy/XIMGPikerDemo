package com.twlk.ximgpiker.helper;
...
//        cr = AppContext.getInstance().getContentResolver();
    ...
    ThreadPoolExecutor tpe = new ThreadPoolExecutor(5, 5, 10, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    String imgPath = null;
                    try {
                        File file = Luban.with(context).load(new File(path)).get();
                        imgPath = file.getPath();
                        bitmap = Glide.with(context).load(imgPath).asBitmap().centerCrop().into(width, height).get();
                    } catch (Exception e) {
                        try {
                            releaseAllSizeCache();
